...
확장 측정(rise/flat/fall_dirty × N=10)도 완료됐다(2026-08-12,
[../slice/out/loop8x/findings.md](../slice/out/loop8x/findings.md)): §12-1 상승
관대화는 작문 에이전트에서 기각(헤지·기권 시나리오 불변), 무변화·게이트 거부
보고는 형태(분량·수치 밀도)로 응답, 명목 BLOCK 26건은 전수 린트 오탐(반올림·
콤마·산술 대조 파생치·R01 과정 설명 오폭 — 4계층 수리). 라벨 규율은 시나리오
불변, 시나리오는 형태를 조절한다.

커널 게이트 2건은 구현됐다(2026-08-12, ADR-0006·0007): 슬라이스 실측 게이트
(실측은 보류하되 선언은 보존 — 행 3분류, 배제 우선)와 부호 규범 게이트(선언만
있던 규범의 배선, 미선언도 실패). 적대 프로브(test_gates.py)로 무발화·발화
양방향 고정.

계약 v0.2도 개정·검증됐다(2026-08-12, §12 + 린트 R13·R14): dirty 재작문 N=10에서
거부 요약 유예 30%→0%, 선언 무출처 인용 12건→0건, 보류 실측 누출 0건 — 결함
목록→조항·게이트→전멸 루프의 2주기째 실증. 교훈: 규범의 최선 구현은 어법
규칙(40% 준수)이 아니라 데이터 구조(값을 주지 않음 — 100% 준수)다.

미결 (우선순위순):
- 실데이터 온보딩 비용 실측 — 지식 획득 병목이 최대 제품 리스크
- δ_min의 유래 부재
- 린트 v0.3 방향: 어휘 매칭 → 구조 슬롯 검사(3라운드 연속 어휘 오탐의 교훈)
- 추세·변곡 연산자, witness 엔진(계약 확정 후)
- 계약 v0.1 조항 1~4의 린트 구현 — N=20에서 100% 준수 확인, 회귀 방지용 백로그
- 장르 축 확장 측정(CFO 외), 주별 연산자 2막 드릴다운(주별 데이터는 원장에 존재)
